...
> 新增工作流：在 `.codebuddy/workflows/{workflow-id}/` 创建 `workflow.md`、`state-template.md` 和 `routing.yaml`，并新建对应 planner 或入口 skill。
...
2. 确认当前阶段状态
3. 通过 `.codebuddy/scripts/todo-state.sh` 启动或完成阶段
4. 不可跳步，不可不做
