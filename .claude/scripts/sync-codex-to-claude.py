#!/usr/bin/env python3
"""Synchronize managed Claude Code configuration into the Claude compatibility mirror."""
...
def transform_text(path: Path) -> None:
    ...
    try:
        ...
        return
    transformed = original.replace(".claude", ".claude").replace("Claude Code", "Claude Code")
...
def main() -> int:
    ...
    if not (root / ".claude").is_dir():
        print(f"sync-codex-to-claude: expected .claude under {root}", file=os.sys.stderr)
        return 2

    if not args.check:
        apply_sync(root)
        print("Synced portable Claude Code config to Claude Code config.")
        return 0
