...
.claude/rules/workflow-routing.md from .claude/workflows/*/routing.yaml.
...
ROUTING_FILE="${PROJECT_ROOT}/.claude/rules/workflow-routing.md"
...
render_generated_block() {
  ...
  if [ -d "$WORKFLOWS_DIR" ]; then
    find "$WORKFLOWS_DIR" -mindepth 2 -maxdepth 2 -name routing.yaml -type f -print | sort | while IFS= read -r metadata; do
      ...
      printf '| `%s` | %s | %s | %s | %s | `.claude/workflows/%s/workflow.md` | `%s` |\n' \
...
if [ "$MODE" = "check" ]; then
  echo "workflow routing is stale; run .claude/scripts/sync-workflow-routing.sh" >&2
  exit 1
fi
