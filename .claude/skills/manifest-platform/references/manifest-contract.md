...
`entrypoint` is resolved relative to the manifest’s directory and must remain inside `.claude/`. `dependsOn` uses exact artifact IDs such as `Skill/research-collector`. A Hook additionally declares `event: Stop` or `event: SessionStart`; it must still be registered in `.claude/hooks.json`.
