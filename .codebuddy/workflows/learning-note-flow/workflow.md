...
状态通过 `[PN]` 阶段标记和 YAML frontmatter 共同记录。每次阶段切换都调用 `.codebuddy/scripts/todo-state.sh`，避免各 skill 手写 `sed` 导致状态漂移：
...
```bash
.codebuddy/scripts/todo-state.sh "$RUN_STATE_FILE" start P2
.codebuddy/scripts/todo-state.sh "$RUN_STATE_FILE" mode P2 freeform "用户选择随性模式"
.codebuddy/scripts/todo-state.sh "$RUN_STATE_FILE" confirm P2 "用户确认研究方向"
.codebuddy/scripts/todo-state.sh "$RUN_STATE_FILE" complete P2
.codebuddy/scripts/todo-state.sh "$RUN_STATE_FILE" start P3
.codebuddy/scripts/todo-state.sh "$RUN_STATE_FILE" skip P3 "随性模式不生成大纲"
```
...
2. 调用状态脚本将运行模式持久化为 `freeform`，再确认并完成阶段 2：
   ```bash
   .codebuddy/scripts/todo-state.sh "$RUN_STATE_FILE" mode P2 freeform "用户选择随性模式"
   .codebuddy/scripts/todo-state.sh "$RUN_STATE_FILE" confirm P2 "用户确认直接组装"
   .codebuddy/scripts/todo-state.sh "$RUN_STATE_FILE" complete P2
   ```
3. 依次启动并跳过阶段 3 和阶段 4；脚本会把选择写入方向调整记录和跳过记录：
   ...
   .codebuddy/scripts/todo-state.sh "$RUN_STATE_FILE" start P3
   .codebuddy/scripts/todo-state.sh "$RUN_STATE_FILE" skip P3 "随性模式不生成大纲"
   .codebuddy/scripts/todo-state.sh "$RUN_STATE_FILE" start P4
   .codebuddy/scripts/todo-state.sh "$RUN_STATE_FILE" skip P4 "随性模式不逐章写作"
   ```
