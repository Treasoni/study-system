...
if [ -d ".codex/platform" ]; then
  copy_dir ".codex/platform" ".claude/platform"
fi
...
find .claude/skills .claude/agents .claude/rules .claude/scripts .claude/platform .claude/workflows \
  -type f \( -name "*.md" -o -name "*.sh" -o -name "*.py" -o -name "*.yaml" \) \
