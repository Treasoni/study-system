...
Options:
  ...
  --init-layout     Create .codex/workflows, workspace/workflow-runs, and workflow-routing.md.
...
Examples:
  .codex/skills/workflow-todo-state/scripts/install.sh ../other-project
  .codex/skills/workflow-todo-state/scripts/install.sh ../other-project --with-skill --init-layout --update-agents
USAGE
}
...
install_state_script() {
  local target_dir="${TARGET_PROJECT}/.codex/scripts"
...
install_skill() {
  local target_skills="${TARGET_PROJECT}/.codex/skills"
...
init_layout() {
  local workflows_dir="${TARGET_PROJECT}/.codex/workflows"
  local rules_dir="${TARGET_PROJECT}/.codex/rules"
...
Next:
  1. Create a workflow definition under .codex/workflows/{workflow-id}/.
  2. Add routing metadata at .codex/workflows/{workflow-id}/routing.yaml.
  3. Run .codex/scripts/sync-workflow-routing.sh.
  4. Create a named state file under workspace/workflow-runs/{task}.workflow.md.
  5. Test: .codex/scripts/todo-state.sh workspace/workflow-runs/{task}.workflow.md start P0
EOF
