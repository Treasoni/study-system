...
```bash
.codex/skills/manifest-platform/scripts/install.sh --target .
```
...
1. Inspect the project’s existing `.codex/` layout and preserve unrelated changes.
...
5. Add dependency IDs only after their target manifests exist. For a Hook, also retain the actual registration in `.codex/hooks.json`.
6. Run `python3 .codex/platform/manifest-registry.py --root . validate` and fix every failure before handoff.

## Reuse

Copy or install this Skill into another project’s `.codex/skills/`, then run its installer against that project. The installer supplies a dependency-free validator, JSON Schema, registry policy, and documentation; use `manifest-registry.py init` to create an initial manifest before refining its capabilities and permissions.
