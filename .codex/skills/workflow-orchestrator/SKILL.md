...
description: 业务工作流实例化器。由 planner 技能调用，接收 workflow_id、topic、project_slug 等参数，按 .codex/workflows/{workflow-id}/state-template.md 生成 workspace/workflow-runs/{run-id}.workflow.md。不直接面向用户；通用状态格式和状态流转由 workflow-todo-state 负责。
---
...
| `.codex/rules/workflow-routing.md` | 汇总可用工作流和适用场景 |
...
`workflow-orchestrator` 优先使用 `.codex/workflows/{workflow-id}/state-template.md`。

兼容旧结构时，可以回退读取：

```text
.codex/skills/workflow-orchestrator/templates/{workflow-id}-todo.md
```
...
1. 读取 `.codex/rules/workflow-routing.md`，确认 `workflow_id` 存在。
2. 定位 `.codex/workflows/{workflow-id}/workflow.md` 和 `state-template.md`。
...
8. 返回 run state file 路径，后续阶段必须通过 `.codex/scripts/todo-state.sh` 更新状态。
...
WORKFLOW_DIR=".codex/workflows/${WORKFLOW_ID}"
...
以 `.codex/rules/workflow-routing.md` 为准。当前项目内的主要工作流：

| Workflow ID | Definition | State Template |
| --- | --- | --- |
| `learning-note-flow` | `.codex/workflows/learning-note-flow/workflow.md` | `.codex/workflows/learning-note-flow/state-template.md` |
| `legacy-note-import-flow` | `.codex/workflows/legacy-note-import-flow/workflow.md` | `.codex/workflows/legacy-note-import-flow/state-template.md` |
| `batch-note-update-flow` | `.codex/workflows/batch-note-update-flow/workflow.md` | `.codex/workflows/batch-note-update-flow/state-template.md` |
...
```bash
.codex/scripts/todo-state.sh "${RUN_STATE_FILE}" start P1
.codex/scripts/todo-state.sh "${RUN_STATE_FILE}" complete P1
.codex/scripts/todo-state.sh "${RUN_STATE_FILE}" skip P3 "原因"
.codex/scripts/todo-state.sh "${RUN_STATE_FILE}" block P2 "原因"
```
