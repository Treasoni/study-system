...
```bash
.codex/scripts/todo-state.sh "${PROJECT_DIR}/todo.md" start P2
.codex/scripts/todo-state.sh "${PROJECT_DIR}/todo.md" complete P2
.codex/scripts/todo-state.sh "${PROJECT_DIR}/todo.md" skip P3 "用户选择随性模式"
.codex/scripts/todo-state.sh "${PROJECT_DIR}/todo.md" block P2 "素材来源不足"
```
...
**触发规则**：阶段 3 正常执行完毕后，todo.md 中阶段 3 状态标记为 ✅，阶段 4 开始执行。
...
3. 调用状态脚本明确跳过阶段 3 和阶段 4：
   ```bash
   .codex/scripts/todo-state.sh "${PROJECT_DIR}/todo.md" skip P3 "用户选择随性模式"
   .codex/scripts/todo-state.sh "${PROJECT_DIR}/todo.md" skip P4 "随性模式不逐章写作"
   ```
...
sed -i '' "s/{total_chapters}/${TOTAL_CHAPTERS}/g" "${WORKSPACE_PATH}/${PROJECT_SLUG}/todo.md"
```
