...
- Phases: collect → write → beautify → [evaluate] → [digest]
...
2. Present summary to user:
   - How many sources collected, how many kept after curation, how many removed
   - Quality stats and which sub-topics are covered
   - Any obvious gaps
3. MUST execute Write tool to mark Phase 1 `[x]` in `{SYSTEM_ROOT}/TODO.md`.
4. **STOP here.** Wait for user to explicitly say "继续" or "进入下一阶段".
...
3. Present draft to user:
   ...
   - Accuracy check
   - Any missing content
4. MUST execute Write tool to mark Phase 2 `[x]` in `{SYSTEM_ROOT}/TODO.md`.
...
4. MUST execute Write tool to mark Phase 4 `[x]` in `{SYSTEM_ROOT}/TODO.md`.
